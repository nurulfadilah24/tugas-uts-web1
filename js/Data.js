...
// Fungsi untuk menampilkan nama user di index
function tampilkanDataLogin() {
  const user = JSON.parse(localStorage.getItem("penggunaLogin"));
  const infoDiv = document.getElementById("infoUser");

  if (user && infoDiv) {
    infoDiv.innerHTML = `👤 ${user.nama} (${user.role})`;
  }
}

// Jalankan otomatis ketika halaman index.html dibuka
document.addEventListener("DOMContentLoaded", tampilkanDataLogin);
function logout() {
